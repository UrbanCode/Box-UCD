...
*	Filename: upgradeAPPandAuthenticate.groovy
 */
...
def url_auth = url + "auth-apis/auth/1.0/authenticate/"
...
if (auth_token.isEmpty()) {
	request.url_auth = url_auth
	// Build the XML using the parameters
	LinkedHashMap<String, String> authenticator = new LinkedHashMap<String, String>()
	authenticator.put("billingID", billing_id)
	authenticator.put("platformID", platform_id)
	authenticator.put("appID", app_id)
	authenticator.put("appVersion", app_version)
	authenticator.put("appAccessKey", app_access_key)
	authenticator.put("userName", username)
	authenticator.put("password", password)
	
	request.params = authenticator
	
	request.authToken = request.wsAuthenticationRequest("NORMAL", false)
}
...
request.createRequest("application-apis/applications/api-version/upgradeApp/", 1, billing_id, parametersObjectList)
